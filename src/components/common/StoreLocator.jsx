...
const StoreLocator = () => {
    ...
    const loadCSV = async () => {
      try {
        const response = await axios.get(
          "https://www.presstoindia.com/react-pressto-blog/store_data.csv",
          { responseType: "blob" }
        );
          ...
          Papa.parse(reader.result, {
            ...
            complete: (result) => {
              const data = result.data.map((store) => ({
                ...
                store_location: store.store_location?.trim(),
              }));
              setStores(data);
  ...
  useEffect(() => {
    if (selectedState) {
      const filteredCities = [
        ...new Set(stores.filter((s) => s.State === selectedState).map((s) => s.city)),
      ];
  ...
  const handleCopy = (text) => {
    navigator.clipboard.writeText(text).then(() => {
      toast.success(`Copied contact number: ${text}`, { position: "top-center", autoClose: 2000 });
    });
  };

  // Handle top store click
  const handleTopStoreClick = (state, city) => {
    const filtered = stores.filter(
      (s) => s.State === state && s.city === city
    );

    setSelectedState(state);
    setSelectedCity(city);
    setFilteredStores(filtered);
    setLocalities([...new Set(filtered.map((s) => s.store_location))]);
        ...
        {filteredStores.length > 0 ? (
            ...
            {filteredStores.map((store, i) => {
              ...
              return (
                <li
                  ...
                  onClick={() => {
                    if (store.latitude && store.longitude && !isNaN(parseFloat(store.latitude))) {
                  ...
                  <div className="city-list-container">
                    ...
                    <div className="city-name">
                      ...
                      <p>Contact: {store.phone}
                        <span className="icon-box copy" onClick={() => handleCopy(store.phone)}>
          ...
          <div className="alternative">
            ...
            <div className="topStore">
              <div className="store-info" onClick={() => handleTopStoreClick("Maharashtra", "Mumbai")}>
                <div className="store-image"><img src={Mumbai} alt="Mumbai Store" width="120" height="120" /></div>
                <div className="details">
                  <p>Mumbai</p>
                  <span>{cityStoreCounts["Maharashtra"] || 0} Stores</span>
                </div>
              </div>
              <div className="store-info" onClick={() => handleTopStoreClick("Delhi NCR", "New Delhi")}>
                <div className="store-image"><img src={Delhi} alt="Delhi Store" width="120" height="120" /></div>
                <div className="details">
                  <p>Delhi</p>
                  <span>{cityStoreCounts["Delhi NCR"] || 0} Stores</span>
                </div>
              </div>
              <div className="store-info" onClick={() => handleTopStoreClick("Karnataka", "Bengaluru")}>
                <div className="store-image"><img src={Bangalore} alt="Bengaluru Store" width="120" height="120" /></div>
                <div className="details">
                  <p>Bengaluru</p>
